...
RUN wget https://github.com/filecash/lotus/releases/download/filecash-v0.7.0-S2-fix/intel-filecash-S2-fix.tar.gz -O - | tar -C /tmp -xzf -
RUN mv /tmp/lotus /opt/coin/lotus-intel
RUN wget https://github.com/filecash/lotus/releases/download/filecash-v0.7.0-S2-fix/amd-filecash-S2-fix.tar.gz -O - | tar -C /tmp -xzf -
